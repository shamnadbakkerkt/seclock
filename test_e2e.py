...
def test_all_e2e_flows():
    ...
    # 4b. Life Certificate (Jeevan Pramaan) Upload
    life_cert_text = """
    GOVERNMENT OF INDIA
    ...
    # 6. Submitting Nominee Shares
    s1, s2, s3 = shares[0], shares[1], shares[2]
    res = client.post("/api/claim/submit-share", json={"nominee_name": "Priya Ramachandran", "share_index": s1["share_index"], "share_hex": s1["share_hex"]})
    assert res.json()["is_reconstructed"] is False

    res = client.post("/api/claim/submit-share", json={"nominee_name": "Anil Nair", "share_index": s2["share_index"], "share_hex": s2["share_hex"]})
    assert res.json()["is_reconstructed"] is False

    res = client.post("/api/claim/submit-share", json={"nominee_name": "Dr. Radhika Nair", "share_index": s3["share_index"], "share_hex": s3["share_hex"]})
    claim_res = res.json()
    ...
    assert res.status_code == 200
    assert res.json()["integrity"]["is_valid"] is True
